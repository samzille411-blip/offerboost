...
export function buildLevelPrompt(level: number): string {
  const base = PREMIUM_SAFETY_PROMPT + "\n请对比【个人简历】与【目标岗位JD】输出优化报告：\n";
  if (level === 1) {
    return `${base}【体验版】按以下结构输出 Markdown：

## 硬伤诊断
（3 条，每条 1-2 句，直击要害）

## 优化示范
（针对最显眼的一段工作经历，150 字以内 STAR 风格改写示范）`;
  }
  if (level === 2) {
    return `${base}【标准版】按以下结构输出 Markdown：

## 致命问题
（3 条，说明为何与 JD 不匹配）

## STAR 法则改写
（2-3 条核心工作经历，每条单独一段，含量化成果，基于用户真实经历合理推演，勿凭空捏造）

## ATS 高频关键词
（5 个 JD 关键词，每条格式：**关键词** — 建议嵌入位置与写法）`;
  }
  return `${base}【冲刺版】包含标准版全部章节，并增加：

## 面试必问题预测
（3 道题，每题含：可能问题、高分回答策略、避坑提示）`;
}
